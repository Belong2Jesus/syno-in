...
function SimpleWebRTC(opts) {
    ...
    var config = this.config = {
            userName: "alian",    // add yatani
            url: 'http://stun2.syno.jp:8887',    // change yatani
            debug: false,
            userNamesEl: '',    // add yatani
            localVideoEl: '',
            remoteVideosEl: '',
    ...
    var item, connection, userNames; // add yatani
...
console.log("rtc:connect:" + connection.socket.sessionid);
        // add yatani
        connection.emit('setName', { userName: self.config.userName } );
    ...
    // add yatani
    connection.on('userInit', function (message) {
console.log(message);
        self.emit('userInit', message);
    });
    connection.on('userAdd', function (message) {
        for ( var id in message ) {
            var user = {};
            user[id] = message[id];
            self.emit('userAdd', user);
        }
    });
    connection.on('userDel', function (message) {
        for ( var id in message ) {
            self.emit('userDel', message);
        }
    });
    connection.on('invited', function (message) {
        self.emit('invited', connection.socket.sessionid);
    });
...
//    video.width = "640"; video.height = "480"; // add yatani
    video.controls = true;
...
// add yatani
SimpleWebRTC.prototype.invite = function (room) {
console.log("lib:invite:" + room);
  this.connection.emit('invite', room);
};
...
module.exports = function (constraints, cb) {
    ...
    var defaultOpts = {video: { mandatory: { minWidth: 800, minHeight: 600}, optional:[] },
                       audio: true};
console.log(2);
    var error;
...
//                {"url": "stun:stun.l.google.com:19303"},
                {"url": "stun:stun.syno.jp:80"},
                {"url": "turn:stun.syno.jp:80", "username":"syno", "credential":"syno"}
//                {"url": "stun:s1.voipstation.jp:3478"},
//                {"url": "stun:157.7.194.224:3478"},
//                {"url": "stun:173.194.72.127:19303"},
//                 {"url": "turn:synou@157.7.194.224:3479", "credential": "synou" }
              ]
            },
            ...
            media: {
                audio: true,
                video: { mandatory: { minWidth: 800, minHeight: 600}, optional:[] }
            },
...
console.log("!" + 4 + this.config.media.video);
    // check for support
    if (!webrtc.support) {
...
WebRTC.prototype.startLocalMedia = function (mediaConstraints, cb) {
    ...
    var constraints = mediaConstraints || {video: qvgaConstraints, audio: true};  // mod yatani
console.log(5);
